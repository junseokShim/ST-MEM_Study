...
from models.encoder.st_mem_fft import ST_MEM_ViT_FFT, TransformerBlock


__all__ = ['ST_MEM_BETA', 'st_mem_beta_vit_small_dec256d4b', 'st_mem_beta_vit_base_dec256d4b']
...
class ST_MEM_BETA(nn.Module):
    '''
    apply the TSA block to the input ECG signal
    the input ECG signal is divided into segments
    the segments are merged into a larger segment
    '''
    def __init__(self,
                 seq_len: int = 2250,
                 patch_size: int = 75,
        ...
        self.encoder = ST_MEM_ViT_FFT(seq_len=seq_len,
                                  patch_size=patch_size,
                                  num_leads=num_leads,
                                  width=embed_dim,
                                  depth=depth,
        ...
        self.dec_pos_embedding = nn.Parameter(torch.randn(1, self.num_patches + 2, decoder_embed_dim))
...
def st_mem_beta_vit_small_dec256d4b(**kwargs):
    model = ST_MEM_BETA(embed_dim=384,
                   depth=12,
                   num_heads=6,
...
def st_mem_beta_vit_base_dec256d4b(**kwargs):
    model = ST_MEM_BETA(embed_dim=768,
                   depth=12,
                   num_heads=12,
                   decoder_embed_dim=256,
                   decoder_depth=4,
